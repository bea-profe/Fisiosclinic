    ...
    public function store(Request $request)
{
    ...
    $request->merge(["fecha_nac" => Carbon::parse($date_clean)->format("Y-m-d H:i:s")]);
    ...
    // Almacenar las horas del doctor
    $this->manageSchedule($user, $schedule_hours);
...
private function manageSchedule($user, $schedule_hours)
{
    // Comprobar y eliminar días y horarios obsoletos
    foreach ($user->schedule_days as $schedule_day) {
        $is_exists_schedule_day = false;
        foreach ($schedule_hours as $schedule_hour) {
            if (sizeof($schedule_hour["children"]) > 0 && $schedule_day->day == $schedule_hour["day_name"]) {
                $is_exists_schedule_day = true;
                $this->manageScheduleHours($schedule_day, $schedule_hour["children"]);
                break;
            }
        }
        if (!$is_exists_schedule_day) {
            $schedule_day->schedules_hours()->delete();
            $schedule_day->delete();
        }
    }

    // Comprobar y agregar nuevos días y horarios
    foreach ($schedule_hours as $schedule_hour) {
        if (sizeof($schedule_hour["children"]) > 0) {
            $schedule_day = $user->schedule_days()->firstOrCreate(
...
private function manageScheduleHours($schedule_day, $children)
{
    foreach ($schedule_day->schedules_hours as $schedules_hour) {
        $is_exists_schedules_hour = false;
        foreach ($children as $child) {
            if ($schedules_hour->doctor_schedule_hour_id == $child["item"]["id"]) {
    ...
    foreach ($children as $child) {
        $schedule_day->schedules_hours()->firstOrCreate(
            ["doctor_schedule_hour_id" => $child["item"]["id"]],
